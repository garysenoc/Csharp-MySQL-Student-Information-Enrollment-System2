    ...
    public partial class MANAGE : Form
    {
        ...
        private void loadStudent()
        {
            ...
            while (rd.Read())
            {
                ...
                listView2.Items[listView2.Items.Count - 1].SubItems.Add(rd["sex"].ToString());
            }
            rd.Close();
        }
        private void enable()
        ...
        private void Show_StudData(string srcID)
        {
            ...
            while (rd.Read())
            {
                ...
                tx26.Text = rd["lrnno"].ToString();
                tx27.Text = rd["gradelevel"].ToString();
                tx28.Text = rd["class"].ToString();
        ...
        private void updateRecord(string srcID)
        {
            ...
            if (pic == "" || pic == null)
            {
                sql = string.Format("UPDATE tbstudent SET fname='{0}', mname='{1}', lname='{2}',telno='{3}', age='{4}', sex='{5}', status='{6}',height='{7}', weght='{8}', citizenship='{9}', birthday='{10}', birthplace='{11}', fathername='{12}',fage='{13}', foccupation='{14}', mothername='{15}', mage='{16}',moccupation='{17}', province='{18}', cityadd='{19}', nameguardian='{20}', gaddress='{21}', persontocontact='{22}',persontocontactaddress='{23}', persontocontactadd='{24}', lrnno='{25}', gradelevel='{26}',class='{27}', semester='{28}', schoolyear='{29}' WHERE id={30}",
         tx1.Text, tx2.Text, tx3.Text, tx4.Text, tx5.Text, tx6.Text, tx7.Text, tx8.Text, tx9.Text, tx10.Text, tx11.Text, tx12.Text, tx13.Text, tx14.Text, tx15.Text, tx16.Text, tx17.Text, tx18.Text, tx19.Text, tx20.Text, tx21.Text, tx22.Text, tx23.Text, tx24.Text, tx25.Text, tx26.Text, tx27.Text, tx28.Text, tx29.Text, tx30.Text, srcID);
                sql_cmd = new MySqlCommand(sql, clsMySQL.sql_con);
            ...
            {
                sql = string.Format("UPDATE tbstudent SET fname='{0}', mname='{1}', lname='{2}',telno='{3}', age='{4}', sex='{5}', status='{6}',height='{7}', weght='{8}', citizenship='{9}', birthday='{10}', birthplace='{11}', fathername='{12}',fage='{13}', foccupation='{14}', mothername='{15}', mage='{16}',moccupation='{17}', province='{18}', cityadd='{19}', nameguardian='{20}', gaddress='{21}', persontocontact='{22}',persontocontactaddress='{23}', persontocontactadd='{24}', lrnno='{25}', gradelevel='{26}',class='{27}', semester='{28}', schoolyear='{29}', pic='{30}' WHERE id={31}",
                         tx1.Text, tx2.Text, tx3.Text, tx4.Text, tx5.Text, tx6.Text, tx7.Text, tx8.Text, tx9.Text, tx10.Text, tx11.Text, tx12.Text, tx13.Text, tx14.Text, tx15.Text, tx16.Text, tx17.Text, tx18.Text, tx19.Text, tx20.Text, tx21.Text, tx22.Text, tx23.Text, tx24.Text, tx25.Text, tx26.Text, tx27.Text, tx28.Text, tx29.Text, tx30.Text, pic, srcID);
                sql_cmd = new MySqlCommand(sql, clsMySQL.sql_con);
                sql_cmd.ExecuteNonQuery();
                MessageBox.Show("Student Data has been update successfully!", "Update Student");
                clearAll();
        ...
        {
            sql = "SELECT * FROM tbstudent WHERE fname LIKE '%" + textBox2.Text + "%' OR mname LIKE '%" + textBox2.Text + "%' OR lname LIKE '%" + textBox2.Text + "%' OR id LIKE '%" + textBox2.Text + "%' OR lrnno LIKE '%" + textBox2.Text + "%' OR citizenship LIKE '%" + textBox2.Text + "%' OR class LIKE '%" + textBox2.Text + "%'OR semester LIKE '%" + textBox2.Text + "%'OR schoolyear LIKE '%" + textBox2.Text + "%'OR cityadd LIKE '%" + textBox2.Text + "%'OR province LIKE '%" + textBox2.Text + "%'OR mothername LIKE '%" + textBox2.Text + "%'OR fathername LIKE '%" + textBox2.Text + "%' OR gradelevel LIKE '%" + textBox2.Text + "%'OR lrnno LIKE '%" + textBox2.Text + "%'";
            sql_cmd = new MySqlCommand(sql, clsMySQL.sql_con);
            MySqlDataReader rd = sql_cmd.ExecuteReader();
            listView2.Items.Clear();
            while (rd.Read())
